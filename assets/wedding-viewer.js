import {isTension,closedTensionGeometry,stoneChannels,channelDisplacement,stoneFrame} from './wedding-settings.js?v=3';
import * as THREE from 'three';
import {OrbitControls} from 'three/OrbitControls.js';
import {GLTFLoader} from 'three/GLTFLoader.js';
import {loadJewelry,weddingGeometry,createWeddingStudio,createStudio,diamondMesh} from './jewelry-studio.js?v=20260921-catalog3';
import {sampleRingProfile} from './wedding-materials.js?v=20260921-blender2';
import {metalMaterial,metalColor} from './wedding-finishes.js?v=3';
...
function shadow(){const c=document.createElement('canvas');c.width=c.height=128;const ctx=c.getContext('2d'),gradient=ctx.createRadialGradient(64,64,0,64,64,64);gradient.addColorStop(0,'rgba(57,49,36,.29)');gradient.addColorStop(.35,'rgba(57,49,36,.15)');gradient.addColorStop(1,'rgba(57,49,36,0)');ctx.fillStyle=gradient;ctx.fillRect(0,0,128,128);const mesh=new THREE.Mesh(new THREE.PlaneGeometry(1,1),new THREE.MeshBasicMaterial({map:new THREE.CanvasTexture(c),transparent:true,depthWrite:false}));mesh.rotation.x=-Math.PI/2;return mesh;}
export class WeddingViewer{
 constructor(stage){this.stage=stage;this.rings=[];this.signatures=[];this.rotating=!matchMedia('(prefers-reduced-motion: reduce)').matches;this.dirty=true;this.visible=true;this.ready=false;this.lastTime=0;this.profiles=new Map();this.contours=new Map();}
 async init(){
  this.renderer=new THREE.WebGLRenderer({antialias:true,alpha:true,preserveDrawingBuffer:true,powerPreference:'high-performance'});this.renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,2));this.renderer.setSize(this.stage.clientWidth,this.stage.clientHeight);this.renderer.outputColorSpace=THREE.SRGBColorSpace;this.renderer.toneMapping=THREE.NeutralToneMapping;this.renderer.toneMappingExposure=.94;this.aniso=this.renderer.capabilities.getMaxAnisotropy();this.stage.append(this.renderer.domElement);this.renderer.domElement.setAttribute('aria-hidden','true');
  ...
  const [base,profiles,gems,studio]=await Promise.all([loadJewelry(),extra('models/wedding-profiles.glb?v=3'),extra('models/wedding-gems.glb?v=3'),createWeddingStudio(this.renderer).catch(()=>createStudio(this.renderer))]);
  this.library=new Map([...base,...profiles,...gems]);this.studio=studio;this.scene.environment=studio.metal;this.stage.dataset.studio='blender';
  ...
  for(const [color,intensity,pos] of [[0xfff7e9,.22,[-20,30,35]],[0xffffff,.13,[28,13,-20]]]){const light=new THREE.DirectionalLight(color,intensity);light.position.set(...pos);this.scene.add(light);}
 ...
 build(state,fit){
  for(let i=0;i<2;i++){
   ...
   ring.rotation.set(Math.PI/2,0,0);ring.rotateOnWorldAxis(new THREE.Vector3(0,1,0),i===0?.94:-1.14);ring.rotateOnWorldAxis(new THREE.Vector3(0,0,1),i===0?-.17:-.035);
  }
  this.state=state;
  for(let i=0;i<state.rings.length;i++){const ring=this.rings[i];ring.position.set(0,0,0);ring.updateMatrixWorld(true);const bounds=new THREE.Box3().setFromObject(ring,true),sign=state.rings.length===1?0:i===0?-1:1;const x=sign*((bounds.max.x-bounds.min.x)/2+.65),z=i===0?-1:1.6;ring.position.set(x,-bounds.min.y,z);const radius=state.rings[i].size/TAU+state.rings[i].height;this.shadows[i].position.set(x,.015,z);this.shadows[i].scale.set(radius*3.1,radius*1.5,1);}
 ...
 ringGeometry(k){
  let geo=weddingGeometry(this.library.get('Wedding_'+k.profile),k.size/TAU,k.height,k.width);const channels=stoneChannels(this,k,this.stonePlan(k));
  if(!isTension(k)&&(k.groove.quantity||k.edge.type!=='none'||k.separations.some(Boolean)||channels.length)){
   ...
   rows.push(rows[0].clone());geo.dispose();geo=new THREE.LatheGeometry(rows,320);
  ...
  if(isTension(k)){geo.dispose();geo=closedTensionGeometry(this,k,stoneSize(k).width);}
   const machined=new Float32Array(geo.attributes.position.count);if(channels.length){const p=geo.attributes.position,n=geo.attributes.normal;for(let i=0;i<p.count;i++){const result=channelDisplacement(k,channels,p.getX(i),p.getY(i),p.getZ(i),n.getX(i),n.getY(i),n.getZ(i));p.setXYZ(i,result.x,result.y,result.z);machined[i]=result.depth;}geo.computeVertexNormals();}
 ...
 addStone(group,k,p,materials){
  ...
  let gem;
  if(GEM_COLORS[p.quality])gem=new THREE.Mesh(source,new THREE.MeshPhysicalMaterial({color:GEM_COLORS[p.quality],metalness:.08,roughness:.035,ior:2.3,clearcoat:1,envMapIntensity:2.6,flatShading:true}));
  else gem=diamondMesh(source,this.studio.diamond);
  ...
  const tension=p.preset==='clamping-open'||(p.preset==='combined'&&k.stone.setting==='tension');
   const top=p.preset==='top'||(k.stone.preset==='combined'&&k.stone.setting==='top'&&p.preset==='combined');
  if(top){const alloy=k.stone.mountingMetal?.split('-')[1]||'white';metal.color=metalColor({color:METALS[alloy]?alloy:'white',grade:585});const basket=new THREE.Mesh(this.library.get(k.stone.mounting==='round6'&&p.cut==='brilliant'?'Basket_6':'Basket_4'),metal);basket.userData.sharedGeometry=true;basket.scale.set(r*(p.cut==='princess'?1.4:1),r,r*(p.cut==='princess'?1.4:1));setting.add(basket);}
  else if(p.preset.includes('section')||p.preset.startsWith('memoire')){
   const n=p.preset==='memoire2'?2:4;
   for(let i=0;i<n;i++){const a=p.preset==='memoire4'?[.48,-.48,Math.PI+.48,Math.PI-.48][i]:TAU*i/n+Math.PI/4,bead=new THREE.Mesh(p.preset==='memoire2'?new THREE.BoxGeometry(r*.16,r*.11,r*1.1):new THREE.SphereGeometry(r*.12,12,8),metal);bead.position.set(Math.cos(a)*r*.9,r*.03,Math.sin(a)*r*.9);setting.add(bead);}
  ...
  setting.quaternion.copy(stoneFrame(p.phi,normal));setting.rotateY((p.size.rotation||0)*Math.PI/180);const lift=top?r*.92:-r*.035;
  setting.position.set(Math.cos(p.phi)*radius+normal.x*lift,py+normal.y*lift,Math.sin(p.phi)*radius+normal.z*lift);group.add(setting);
 }
 engraving(k){
 ...
 tick(time){if(!this.ready||document.hidden||!this.visible||time-this.lastTime<32)return;const dt=Math.min((time-this.lastTime)/1000,.1);this.lastTime=time;this.controls.autoRotate=this.rotating;const changed=this.controls.update(dt);if(!changed&&!this.dirty&&this.hasFrame)return;this.renderer.render(this.scene,this.camera);this.dirty=false;this.hasFrame=true;this.stage.classList.add('is-bereit');}
