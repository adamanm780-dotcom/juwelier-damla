...
SUB_CSS = """
    ...
    /* Gilt fuer beide Traeger: manche Baender fuehren ein Video, andere
       ein Bild. Die Regel gehoert deshalb hierher und nicht in das CSS
       einer einzelnen Seite — dort fehlte sie und das Bild lief nicht
    ...
    /* Beim Hover gibt der Schleier etwas frei, aber nur so viel, dass
       der Fliesstext ueber allen sechs Motiven noch auf mindestens
       4,5:1 kommt — bei 0.86 rutschte die Ringweiten-Karte auf 4,36. */
    .service-card--foto:hover::before,
    .service-card--foto:focus-within::before { opacity: 0.92; }
    ...
    /* Native carousel by default; the desktop scene budgets space for its copy. */
    .ring-scroll { position: relative; height: auto; padding: 0; }
    .ring-scroll__pin { position: static; display: flex; flex-direction: column; height: auto; padding-bottom: 28px; }
    .ring-scroll__head { flex: none; text-align: center; padding: clamp(24px,4svh,65px) var(--pad-x) 18px; }
    .ring-scroll__head .eyebrow { display: block; }
    .ring-scroll__head .section-title { margin-top: 10px; font-size: clamp(2.25rem,3.5vw,4.8rem); }
    .ring-scroll__head .gold-bar { margin: 16px auto 0; }
    .ring-scroll__viewport { display: flex; min-height: 0; min-width: 0; overflow-x: auto; scroll-snap-type: x mandatory; }
    .ring-scroll__viewport:focus-visible { outline: 2px solid var(--gold); outline-offset: -3px; }
    .ring-scroll__track { display: flex; align-items: stretch; min-height: 0; width: 100%; }
    .ring-panel { flex: 0 0 90%; min-width: 0; min-height: 0; display: grid; place-items: center; padding: 24px clamp(18px,4vw,64px); scroll-snap-align: center; }
    .ring-panel__inner { position: relative; display: grid; justify-items: center; align-content: center; text-align: center; width: min(1200px,100%); min-width: 0; min-height: 0; gap: 22px; }
    .ring-panel__img { display: block; width: 100%; height: clamp(240px,44svh,600px); max-width: 860px; min-height: 0; object-fit: contain; filter: drop-shadow(0 18px 24px #96742a26); }
    .ring-panel__body { position: relative; min-width: 0; width: 100%; overflow-wrap: anywhere; }
    .ring-panel__name { font-family: var(--font-serif); font-style: italic; font-weight: 400; font-size: clamp(2rem,3vw,3.2rem); line-height: 1.15; color: var(--ink); }
    .ring-panel__body .gold-bar { margin: 13px auto 15px; }
    .ring-panel__desc { max-width: 44ch; margin: 0 auto; font-size: 1rem; font-weight: 300; line-height: 1.8; color: var(--ink-soft); }
    .ring-scroll__nav { flex: none; display: flex; align-items: center; justify-content: center; gap: 12px; padding: 18px 0; }
    .ring-scroll__dot { width: 8px; height: 8px; border-radius: 50%; border: 1px solid var(--gold); background: transparent; }
    ...
    @media(min-width:901px) and (min-height:600px) and (prefers-reduced-motion:no-preference) {
      .ring-scroll:not(.ring-scroll--native) { height: 340svh; }
      .ring-scroll:not(.ring-scroll--native) .ring-scroll__pin { position: sticky; top: var(--nav-h); height: calc(100svh - var(--nav-h)); padding-bottom: 0; }
      .ring-scroll:not(.ring-scroll--native) .ring-scroll__viewport { flex: 1 1 0; overflow: hidden; scroll-snap-type: none; }
      .ring-scroll:not(.ring-scroll--native) .ring-scroll__track { height: 100%; will-change: transform; }
      .ring-scroll:not(.ring-scroll--native) .ring-panel { flex-basis: 100%; height: 100%; padding-block: 12px; }
      .ring-scroll:not(.ring-scroll--native) .ring-panel__inner { height: 100%; grid-template-rows: minmax(0,1fr) auto; align-content: stretch; gap: clamp(12px,2svh,28px); }
      .ring-scroll:not(.ring-scroll--native) .ring-panel__img { width: 100%; height: 100%; max-height: none; }
    }
    @media(min-width:901px) and (min-height:600px) and (max-height:800px) {
      .ring-scroll:not(.ring-scroll--native) .ring-panel__inner { grid-template-columns: minmax(0,1.2fr) minmax(0,1fr); grid-template-rows: minmax(0,1fr); gap: 40px; align-items: center; }
      .ring-scroll__head { padding-top: 22px; }
    }
...
%s
    ...
    (function () {
      ...
      var small  = window.matchMedia('(max-width: 900px), (max-height: 599px)');
      ...
      function decide() {
        root.classList.remove('ring-scroll--native');
        // Enlarged text gets a naturally sized carousel instead of a clipped scene.
        var copyHeight = Math.max.apply(null, Array.from(panels, function(panel) {
          return panel.querySelector('.ring-panel__body').offsetHeight;
        }));
        var available = view.clientHeight;
        var sideBySide = getComputedStyle(panels[0].firstElementChild).gridTemplateColumns.split(' ').length > 1;
        var crowded = !small.matches && !reduce.matches && available < copyHeight + (sideBySide ? 24 : 170);
        root.classList.toggle('ring-scroll--native', crowded);
        if (small.matches || reduce.matches || crowded) {
          pinned = false;
      ...
      if (document.fonts) document.fonts.ready.then(decide);
...
%(script)s
%(extrabody)s
<script src="assets/site-design.js?v=20260906-layout" defer></script>
</body>
</html>
"""
...
KF_HEAD = '  <style>\n%s  </style>\n  <link rel="stylesheet" href="atelier.css?v=20260921-catalog3">' % KF_CSS
...
KF_BODY_ENDE = """
  ...
  <script type="module" src="assets/konfigurator.js?v=20260921-catalog3"></script>
"""
