...
The profile/gem sources are the .blend files in tools. Rebuild with Blender --background --python tools/build-wedding-profiles.py, tools/build-wedding-gems.py and tools/build-wedding-studios-v4.py (run again with -- --diamond) and tools/build-wedding-settings.py. Browser geometry preserves measured ring dimensions and adds original machining geometry for grooves, channels and closed tension openings. The 13 finish JPGs depict the same viewer materials.
...
## September 22 rendering revision

Separate original Blender HDR environments for metal and gemstones give warm, graduated metal reflections and stronger facet contrast. Six cut-specific Blender basket meshes replace generic prongs. Flush/bead settings receive modeled seats with polished floors and smoothed UV seams. Colored transparent stones use absorption in the internal refraction shader; black diamonds retain an opaque reflective material. Finish thumbnails use the same revised lighting.
